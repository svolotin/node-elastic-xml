...
async function getXmlFromExternal(res, id) {
  try {
    const resp = await search(query('spatialfeatures', { id: id }))
...
async function getXml(res, id) {
  try {
    const spatialfeature = search(query('spatialfeatures', { id: id }))
    const pointset = search(query('shapes', { property: id }))
